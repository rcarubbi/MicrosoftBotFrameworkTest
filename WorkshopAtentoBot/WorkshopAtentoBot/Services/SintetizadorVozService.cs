...
namespace WorkshopAtentoBot.Services
{
    ...
    public class SintetizadorVozService
    {
        ...
        public static async Task<byte[]> Dizer2(string texto)
        {
            ...
            request.AddHeader("UserAgent", "SintetizadorVozService");
            request.AddHeader("X-MICROSOFT-OutputFormat", formatName);
            request.AddHeader("X-Search-AppId", "E8767073394F41BF9C9275DDF3C64B0F");
            request.AddHeader("X-Search-ClientID", "1ECFAE91408841A480F00935DC390960");
            request.AddHeader("Authorization", "Bearer " + accessToken);
            request.AddBody(body);
            IRestResponse postResponse = await client.ExecuteTaskAsync<object>(request);
         
             
            MemoryStream ms = new MemoryStream(postResponse.RawBytes);
